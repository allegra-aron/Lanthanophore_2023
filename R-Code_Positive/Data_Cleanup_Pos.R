...
ft <- read.csv('~/Dropbox/2023-Lanthanide_Nature/R-Code_Positive/Data_Cleanup_Inputs/12012022_pos_mode_1_iimn_gnps_quant.csv', header = T, check.names = F)
md <- read.csv('~/Dropbox/2023-Lanthanide_Nature/R-Code_Positive/Data_Cleanup_Inputs/12022022_metadata_pos.csv', header = T, check.names = F, sep = ',')
...
write.csv(new_md, "~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/lathanide_positive_md_new.csv",row.names = TRUE)

new_md <- read_csv('~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/lathanide_positive_md_new.csv') %>% column_to_rownames("filename")
new_md <- new_md[, -1]
...
write.csv(new_md, '~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/md_new_pos.csv', row.names=TRUE)
...
write.csv(Blank_removal,file.path(paste0('~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/Blank_removed_0.3_07282023.csv')),row.names =TRUE)
...
write.csv(Imputed,file.path(paste0('~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/Imputed_QuantTable_filled_with_',Cutoff_LOD,'_CutOff_Used_',Cutoff,'_07282023.csv')),row.names =TRUE)
...
write.csv(Normalized_data_TIC, file.path('~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/Normalised_Quant_table_pos.csv'),row.names =TRUE)
...
write.csv(Imp_clrt, "~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/CLR_Scaled_lanthanide_positive.csv",row.names = TRUE)

################## Multivariate Statistical Analysis - Principal Component Analysis - PCA ############################

metadata_pos <- read_csv("~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/md_new_pos.csv")
clr_ft_pos <- read_csv("~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/CLR_Scaled_lanthanide_positive.csv")
  
data_merge <- metadata_pos %>% dplyr::select("filename", "ATTRIBUTE_shorthand") %>% 
  left_join(clr_ft_pos) %>% 
  column_to_rownames("filename")


res.pca <- data_merge %>% dplyr::filter(ATTRIBUTE_shorthand != "media") %>%
  dplyr::select(-ATTRIBUTE_shorthand) %>% 
  prcomp(scale = FALSE) #TRUE if scaling was not performed before
fviz_eig(res.pca)

pca <- fviz_pca_ind(res.pca, col.ind = (data_merge %>% dplyr::filter(ATTRIBUTE_shorthand != "media"))$ATTRIBUTE_shorthand, addEllipses = TRUE, label = "none")
pca
ggsave(file="~/Dropbox/2023-Lanthanide_Nature/depricated/code_check/pca_pos.svg", plot=pca, width=10, height=8)

pc_loadings <- res.pca$rotation
pc_loadings <- as_tibble(pc_loadings, rownames = "feature")
pc_loadings

fviz_pca_var(res.pca, select.var = list(contrib = 10), col.var = "contrib", # Color by contributions to the PC
